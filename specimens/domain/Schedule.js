import { ConflictDetector, Section } from '../scheduler/SchedulerEngine.js';
...
export class Schedule {
  constructor(id, semesterId, userId, name, description, status, isFavorite, isPrivate = false) {
    this.id = id;
  ...
  hasConflict(newCourse) {
    ...
    const newSection = new Section(
      newCourse.sectionGroup,
      newCourse.schedule,
      `${newCourse.enrolledCurrent}/${newCourse.enrolledTotal}`,
      newCourse.status
    );
    // Add extra props if needed, though ConflictDetector only uses getParsedSchedule()
    newSection.courseCode = newCourse.courseCode;
    newSection.courseName = newCourse.courseName;

    return this.courses.some(existingCourse => {
      const existingSection = new Section(
        existingCourse.sectionGroup,
        existingCourse.schedule,
        `${existingCourse.enrolledCurrent}/${existingCourse.enrolledTotal}`,
        existingCourse.status
      );
      existingSection.courseCode = existingCourse.courseCode;
      existingSection.courseName = existingCourse.courseName;
